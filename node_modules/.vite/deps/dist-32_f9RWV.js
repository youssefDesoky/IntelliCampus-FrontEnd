...
function findDiffStart(a, b, pos) {
	for (let i$1 = 0;; i$1++) {
		if (i$1 == a.childCount || i$1 == b.childCount) return a.childCount == b.childCount ? null : pos;
		let childA = a.child(i$1), childB = b.child(i$1);
		if (childA == childB) {
			pos += childA.nodeSize;
			continue;
		}
...
function compareDeep(a, b) {
	...
	if (array) {
		...
		for (let i$1 = 0; i$1 < a.length; i$1++) if (!compareDeep(a[i$1], b[i$1])) return false;
	} else {
...
var Mark$1 = class Mark$1 {
	...
	*/
	static sameSet(a, b) {
		if (a == b) return true;
		if (a.length != b.length) return false;
		for (let i$1 = 0; i$1 < a.length; i$1++) if (!a[i$1].eq(b[i$1])) return false;
		return true;
	}
	/**
...
var ContentMatch = class ContentMatch {
	...
	*/
	compatible(other) {
		for (let i$1 = 0; i$1 < this.next.length; i$1++) for (let j = 0; j < other.next.length; j++) if (this.next[i$1].type == other.next[j].type) return true;
		return false;
	}
	/**
	...
	*/
	toString() {
		...
		function scan(m) {
			seen.push(m);
			for (let i$1 = 0; i$1 < m.next.length; i$1++) if (seen.indexOf(m.next[i$1].next) == -1) scan(m.next[i$1].next);
		}
		scan(this);
		return seen.map((m, i$1) => {
			let out = i$1 + (m.validEnd ? "*" : " ") + " ";
			for (let i$2 = 0; i$2 < m.next.length; i$2++) out += (i$2 ? ", " : "") + m.next[i$2].type.name + "->" + seen.indexOf(m.next[i$2].next);
			return out;
		}).join("\n");
	}
};
...
function clearIncompatible(tr, pos, parentType, match = parentType.contentMatch, clearNewlines = true) {
	...
	for (let i$1 = replSteps.length - 1; i$1 >= 0; i$1--) tr.step(replSteps[i$1]);
}
...
function canAppendWithSubstitutedLinebreaks(a, b) {
	...
	for (let i$1 = 0; i$1 < b.childCount; i$1++) {
		let child = b.child(i$1);
	...
	*/
	replace(tr, content = Slice.empty) {
		...
		for (let i$1 = 0; i$1 < ranges.length; i$1++) {
			let { $from, $to } = ranges[i$1], mapping = tr.mapping.slice(mapFrom);
			tr.replaceRange(mapping.map($from.pos), mapping.map($to.pos), i$1 ? Slice.empty : content);
			if (i$1 == 0) selectionToInsertionEnd$1(tr, mapFrom, (lastNode ? lastNode.isInline : lastParent && lastParent.isTextblock) ? -1 : 1);
		}
	}
	/**
		...
		for (;;) {
			...
			for (let i$1 = 0; i$1 < this.config.plugins.length; i$1++) {
				...
				if (plugin.spec.appendTransaction) {
					...
					if (tr && newState.filterTransaction(tr, i$1)) {
						tr.setMeta("appendedTransaction", rootTr);
						if (!seen) {
							seen = [];
							for (let j = 0; j < this.config.plugins.length; j++) seen.push(j < i$1 ? {
								state: newState,
								n: trs.length
							} : {
								state: this,
	...
	parseRange(from, to, base$1 = 0) {
		...
		for (let offset = base$1, i$1 = 0;; i$1++) {
			...
			if (fromOffset > -1 && (end > to || i$1 == this.children.length - 1)) {
				to = end;
				for (let j = i$1 + 1; j < this.children.length; j++) {
...
function sameOuterDeco(a, b) {
	if (a.length != b.length) return false;
	for (let i$1 = 0; i$1 < a.length; i$1++) if (!a[i$1].type.eq(b[i$1].type)) return false;
	return true;
}
function rm(dom) {
...
function mapChildren(oldChildren, newLocal, mapping, node, offset, oldOffset, options) {
	...
	if (mustRebuild) {
		...
		for (let i$1 = 0, j = 0; i$1 < built.children.length; i$1 += 3) {
			let from = built.children[i$1];
			while (j < children.length && children[j] < from) j += 3;
			children.splice(j, 0, built.children[i$1], built.children[i$1 + 1], built.children[i$1 + 2]);
		}
	}
...
function removeOverlap(spans) {
	...
	for (let i$1 = 0; i$1 < working.length - 1; i$1++) {
		...
		if (span.from != span.to) for (let j = i$1 + 1; j < working.length; j++) {
			let next = working[j];
...
var DOMObserver = class {
	...
	registerMutation(mut, added) {
		...
		if (mut.type == "childList") {
			...
			if (ie$1 && ie_version <= 11 && mut.addedNodes.length) for (let i$1 = 0; i$1 < mut.addedNodes.length; i$1++) {
...
function readDOMChange(view, from, to, typeOver, addedNodes) {
	...
	let mkTr = (base$1) => {
		...
		if (parse.sel) {
			...
			if (sel$1 && !(chrome && view.composing && sel$1.empty && (change.start != change.endB || view.input.lastChromeDelete < Date.now() - 100) && (sel$1.head == chFrom || sel$1.head == tr.mapping.map(chTo) - 1) || ie$1 && sel$1.empty && sel$1.head == chFrom)) tr.setSelection(sel$1);
		}
		if (compositionID) tr.setMeta("composition", compositionID);
...
var EditorView = class {
	...
	*/
	...
	updateDraggedNode(dragging, prev) {
		...
		if (this.state.doc.nodeAt(sel.from) == sel.node) found$1 = sel.from;
		else {
			let movedPos = sel.from + (this.state.doc.content.size - prev.doc.content.size);
			if ((movedPos > 0 && this.state.doc.nodeAt(movedPos)) == sel.node) found$1 = movedPos;
		}
...
function generateHTML(doc$2, extensions) {
	const schema = getSchema(extensions);
	return getHTMLFromFragment(Node.fromJSON(schema, doc$2).content, schema);
}
...
function generateText(doc$2, extensions, options) {
	const { blockSeparator = "\n\n", textSerializers = {} } = options || {};
	const schema = getSchema(extensions);
	return getText(Node.fromJSON(schema, doc$2), {
		blockSeparator,
		textSerializers: {
...
function simplifyChangedRanges(changes) {
	...
	return uniqueChanges.length === 1 ? uniqueChanges : uniqueChanges.filter((change, index) => {
		return !uniqueChanges.filter((_, i$1) => i$1 !== index).some((otherChange) => {
...
//# sourceMappingURL=dist-32_f9RWV.js.map