...
import TodayReminderItem from "./TodayReminderItem";

// Icons
import BellIconDark from "../../../../components/icons/BellIconDark";
import RightArrowIcon from "../../../../components/icons/ArrowRightIcon";
...
export default function TodayReminders({ reminders, className }) {
    return (
        <div className={`p-6 bg-surface-bg-light dark:bg-surface-bg-dark border border-default-border-light dark:border-default-border-dark rounded-lg ${className}`}>
            ...
            <menu className="flex flex-col gap-3 mb-8" aria-labelledby="today-reminders-header">
                {reminders.length === 0 ? (
                    ...
                    reminders.map((reminder) => (
                        <TodayReminderItem key={reminder.id} reminder={reminder} />
                    ))
                )}
            </menu>
