...
export default function StudentScoreHeatmap({ className = "", data = [], downloadUrl }) {
  if (!data || data.length === 0) {
    return (
      <ChartCard title="Student Score Heatmap" icon={<TableIcon size={20} />} className={className} downloadUrl={downloadUrl}
        chartType="bar" chartData={[]} categoryField="student" series={[{ field: "score", name: "Score" }]}>
    ...
    <ChartCard title="Student Score Heatmap" icon={<TableIcon size={20} />} className={className} downloadUrl={downloadUrl}
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr>
              <th className="text-left p-2 text-text-tertiary-default-light dark:text-text-tertiary-default-dark font-medium sticky left-0 bg-bg-surface-primary-default-light dark:bg-bg-surface-primary-default-dark z-10">Student</th>
          ...
          <tbody>
            {data.map((row) => (
              <tr key={row.student} className="border-t border-border-primary-default-light dark:border-border-primary-default-dark">
                <td className="text-left p-2 text-text-primary-default-light dark:text-text-primary-default-dark font-medium sticky left-0 bg-bg-surface-primary-default-light dark:bg-bg-surface-primary-default-dark">{row.student}</td>
                {assessmentKeys.map((key) => {
