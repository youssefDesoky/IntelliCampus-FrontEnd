...
export async function fetchRemindersByDay(selectedDay) {
  ...
  const url = `/api/instructor/reminders?selectedDay=${dateStr}`;
...
export async function createReminder(reminderData) {
  return apiClient('/api/instructor/reminders', {
    method: 'POST',
...
export async function updateReminder(reminderId, reminderData) {
  return apiClient(`/api/instructor/reminders/${reminderId}`, {
    method: 'PUT',
    body: JSON.stringify(reminderData),
  });
}
...
export async function deleteReminder(reminderId) {
  await apiClient(`/api/instructor/reminders/${reminderId}`, { method: 'DELETE' });
  return true;
}
