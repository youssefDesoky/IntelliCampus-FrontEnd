...
export default function AdminInfoCard({ user = {}, loading = false }) {
    const infoFields = [
        { label: "Admin ID", value: user.adminCode || user.adminId || user.id || "–", icon: HashIcon },
        { label: "Role", value: user.role || "Administrator", icon: UserTieIcon },
        { label: "Department", value: user.department || user.departmentName || "–", icon: BuildingIcon },
        { label: "Faculty", value: user.faculty || user.facultyName || "–", icon: BookIcon },
        { label: "Nationality", value: user.nationality || "–", icon: StarIcon },
    ];
