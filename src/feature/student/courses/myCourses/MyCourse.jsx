...
export default function MyCourse({ course, role, viewMode, isMobile }) {
    ...
    // Progress calculation differs by role
    const progress = isInstructor
        ? (() => {
            const weeksCompleted = course.weeksCompleted ?? 0;
            const totalWeeks = course.weeks?.length ?? 0;
            return totalWeeks > 0 ? Math.round((weeksCompleted / totalWeeks) * 100) : 0;
        })()
        : course.progress ?? (course.weeks > 0 ? Math.round((course.weeksCompleted / course.weeks) * 100) : null);
            ...
            <div className={`${isMobile || viewMode === "grid" ? "col-span-5" : "col-span-4"}`}>
                <div className={`flex ${isMobile || viewMode === "grid" ? "flex-row-reverse" : "flex-col"} justify-between mb-2`}>
                    <div className="flex gap-3 items-center mb-2">
                        <span className={`px-2 py-1 rounded-full border text-text-primary-active-light dark:text-text-primary-active-dark text-xs font-semibold ${isMobile ? "hidden" : ""}`}>
                ...
                {/* Instructor: stats row (numOfStudents) */}
                {isInstructor && (
                ...
                {/* Student: stats row (attendance, assignments, grade) – currently commented out */}
                {!isInstructor && (
                    <div className={`flex flex-row ${isMobile || viewMode === "grid" ? "gap-3" : "gap-6"} text-sm text-text-secondary-active-light dark:text-text-secondary-active-dark mt-6`}>
                        <div className="flex items-center gap-2">
                            {course.attendance != null ? (
