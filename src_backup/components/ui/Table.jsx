                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between px-3 pt-3">
                        <div className="hidden sm:block text-center text-sm text-text-secondary-light dark:text-text-secondary-dark">
                            {(hasPagingLimit && rawData.length > displayRowLimit)
                                                    ? (itemsLabel
                                                        ? `${t('pagination.showing')} ${(currentPage - 1) * displayRowLimit + 1}–${Math.min(currentPage * displayRowLimit, rawData.length)} ${t('pagination.of')} ${rawData.length} ${itemsLabel}`
                                                        : `${t('pagination.showingFirst', { count: displayRowLimit })} ${t('pagination.of')} ${rawData.length} ${t('pagination.rows')}`)
                                : paginationSummary || ""}
                        </div>
