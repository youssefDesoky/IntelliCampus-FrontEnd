...
export default function EditProfileForm({ isOpen, onClose, user, onSaved }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const newErrors = {};
        if (!fullName.trim()) newErrors.fullName = t('profile.editFormValidationFullNameRequired');
        if (!phoneNumber.trim()) {
            newErrors.phoneNumber = t('profile.editFormValidationPhoneRequired');
        } else if (!egyptianPhoneRegex.test(phoneNumber.trim())) {
            newErrors.phoneNumber = t('profile.editFormValidationPhoneInvalid');
        }
        if (!address.trim()) newErrors.address = t('profile.editFormValidationAddressRequired');
        ...
        try {
            ...
            showError(err?.message || t('profile.errorUpdate'));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        ...
        >
            <div className="space-y-5">
                ...
                />
                <InputItem
                    label={t('profile.editFormPhoneLabel')}
                    type="tel"
                    name="phoneNumber"
                ...
                />
                <div>
                    <label htmlFor="address" className="block mb-2 font-bold text-sm text-text-primary-default-light dark:text-text-primary-default-dark">{t('profile.editFormAddressLabel')}</label>
                    <TextArea
                        name="address"
